...
- (void)subtractTime {
    ...
    if (seconds == 0) {
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Time's up !"
                                                        message:[NSString stringWithFormat:@"You have made %i push-ups.", count]
                                                       delegate:self
...
- (IBAction)renunta {
    ...
    Workout* workout = [workouts objectAtIndex:0];
    
    [DatabaseHelper insertWorkoutExercise:workout.uId :@"Test" :user.userUUID :count]; */
